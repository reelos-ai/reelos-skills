...
- 把中国诗词、散文或古意短句集转成经过内容审美评分、双色书法、GPT Image、TTS 与 FFmpeg 合成的视频；
...
| Skill | 适合什么时候用 | 主要输出 |
| --- | --- | --- |
| `chinese-calligraphy-video` | 你想制作中国诗词、散文或短句集视频，并完成内容审美评分、传统双色书法、GPT Image 背景、TTS 与确定性 FFmpeg 合成。 | 设计清单、书法素材、配音、H.264/AAC 成片、审阅帧 |
...
```text
使用 $chinese-calligraphy-video 分析这首诗的内容审美，比较方向，并生成带 TTS 的双色书法视频。
```
