...
def build_half_second_notes_zh(metadata: dict, chapters: object, title: str, author: str, mode: str = "baopo") -> str:
    ...
    lines.append("\n## 提问链")
    questions = [
        "读前问: 我现在最想改掉的旧第一反应是什么？",
        "读中问: 作者如何解释“我知道但做不到”的问题？",
        "读中问: 哪些章节是在定义机制，哪些只是换场景重复？",
        "读中问: 这个模型和“刺激-选择-反应”有什么差别？",
        "读后问: 我能写出哪一句“当 X 时，我做 Y，因为 Z”的脚本？",
        "应用问: 7 天后我用什么信号判断它真的有用？",
    ]
...
def build_generic_notes_zh(metadata: dict, chapters: object, full_text: str, title: str, author: str, mode: str) -> str:
    ...
    lines.append("\n## 提问链")
    generic_questions = [
        "读前问: 我为什么需要这本书，它要解决我的哪个具体问题？",
        "定位问: 作者反复讲的是一件事、一个概念，还是多个独立主题？",
        "结构问: 哪些章节在定义概念，哪些章节只是提供案例？",
        "证据问: 作者用什么论据证明这个概念可靠？",
        "迁移问: 这个概念能迁移到我的哪个场景，边界在哪里？",
        "行动问: 我能用它做一个什么 7 天实验？",
    ]
