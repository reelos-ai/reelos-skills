...
4. 将选中动作和候选分数写入 `MotionDecisionPlan`。
5. 交给特效师补充英雄时刻、辅助效果、转场连续性、性能降级和效果预算。
6. 将确认后的主动作与增强动作写入 Beat Motion Map。
7. 低清全片预览后进入终审验收，按问题类型定点返修。

不要先挑“好看的动画”再寻找适合它的口播。候选效果必须从当前 beat 的关系类型反推。

## 与特效师及终审的边界

- 动效导演负责全片运动骨架和镜头语法，不负责逐个堆叠视觉特效。
- 自动动效导演负责为每个 beat 选择语义最准确的主动作。
- 特效师读取 `MotionDecisionPlan`，生成 `VFXEnhancementPlan`，只增强表达，不改写主语义。
- 终审验收读取低清全片预览和关键帧，生成 `FinalAcceptanceReport`，不以“感觉不错”代替评分。
- 特效师不得擅自把图表改成大字、把因果改成粒子氛围，或用强转场掩盖信息结构。
- 最终分数不低于 85 且没有硬门槛问题，才进入终版渲染。

详细规则见 `vfx-supervisor.md` 和 `final-acceptance.md`。
