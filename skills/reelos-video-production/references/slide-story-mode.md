...
满足以下任一条件才进入 Slide Story：

- 用户明确指定 Slide Story、slide/slides mode、演示页叙事或某个 Slide Story 模板 ID。
- 交付物是可播放 Web Deck、路演式视频，或 Web Deck + MP4 双交付。
- 内容需要按“判断 → 证据 → 方法 → 行动”逐页建立论证，并明确要求演示页视觉语法。

以下情况不自动进入：

- 只要求整理 PPT 文案、提纲、卡片或页面结构。
- 只提供演示网站作为视觉参考，但最终要的是普通口播视频。
- 明确要求 SketchTalk 竖屏强观点模式，且没有 Deck 交付物。

冲突时按 `显式模式 > 交付物 > 内容形态 > 视觉关键词` 消解。Slide Story 可以使用 SketchTalk 的黑红白主题，但不能同时套两套页面层级。
...
- 默认启用 `auto-motion-director.md`：每页按当前 TTS beat 生成 2-3 个动效候选并评分；页面主动作必须达到 75 分，且相邻两页不得连续重复同一种主效果超过两次。
...
先读取 `slide-story-templates.md`，选择一个模板 ID，再按其中的 `SlideStoryPlan` 契约填充页面。路由、模板和主题必须分别记录，不得用视觉风格代替叙事模板。

规划规则：

- 8–16 页适合演示稿；20–90 秒宣传片通常 6–8 页。
...
- 图表、路径、打字和转场必须由语义触发：趋势才画曲线，因果才走路径，输入行为才打字；没有合格候选时宁可使用稳定排版，不添加装饰性运动。
